...
namespace MimeDetective.Tests.Experimental
{
    public class BinaryFileTypeSearch
    {
		...
		public async Task FromStream()
		{
			...
			{
				var fileType = await fileStream.GetFileTypeBinaryAsync();
		...
		*/
		...
		public async Task StreamShouldBeDisposed(string path, string ext)
		{
			...
			var fileType = await fileStream.GetFileTypeBinaryAsync(shouldDisposeStream: true);
